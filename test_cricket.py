import unittest
from cricket import is_even, batsman_cross, swap_bowler, new_over, end_of_over_check, latest_delivery_completed, \
    increment_balls_bowled_by_bowler, set_bowler_completed_over_count
...
class TestCricket(unittest.TestCase):
    ...
    def test_second_delivery_of_over_completed(self):
        self.assertEqual(latest_delivery_completed(1), 2)

    def test_sixth_delivery_of_over_completed(self):
        self.assertEqual(latest_delivery_completed(5), 6)

    def test_first_ball_in_over_count(self):
        self.assertEqual(increment_balls_bowled_by_bowler(1, {'overs': 0}, 0), "0.1")

    def test_second_ball_in_over_count(self):
        self.assertEqual(increment_balls_bowled_by_bowler(2, {'overs': 0}, 0), "0.2")

    def test_sixth_ball_in_over_count(self):
        self.assertEqual(increment_balls_bowled_by_bowler(6, {'overs': 0}, 0), "0.6")

    def test_first_ball_in_second_over_count(self):
        self.assertEqual(increment_balls_bowled_by_bowler(1, {'overs': 1}, 1), "1.1")

    def test_fifth_ball_in_second_over_count(self):
        self.assertEqual(increment_balls_bowled_by_bowler(5, {'overs': 1}, 1), "1.5")

    def test_bowler_completes_first_over(self):
        self.assertEqual(set_bowler_completed_over_count(
